...
Во время работы приложения нет `warning`ов и предупреждений о неправильном вызове методов, `API` и прочего.
В пользовательском коде не используется `deprecated` API.
Во время работы приложения в логах не логгируются ошибки, а также приложение не падает при любых сценариях использования, кроме случаев предусмотреных спецификацией

## Именование

### Название переменных, параметров, свойств и методов начинается со строчной буквы и записываются в нотации [СamelСase](https://ru.wikipedia.org/wiki/CamelCase) 
...
Если в синхронной функции вызывается `async`-функция, то возращаемый `Promise` должен обрабатывает ошибочную ситуацию `.catch()`
...
### Отсутсвие операций записи на `HTTP GET` методах
...
Если требуется сконкатенировать две переменные со строками внутри, то используется оператор конкатенации (`+`), вместо оператора шаблонной строки
